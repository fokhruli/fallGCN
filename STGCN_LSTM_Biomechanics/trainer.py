...
from stgcn import SGCN_LSTM_Fused, FusedModelTrainer  # Updated imports
import pdb
import os
import logging
import sys
...
logging.info(f"Training instances: {len(train_pose_x)}")
...
trainer = FusedModelTrainer(
    ...
    train_biomech_x=train_biomech_x,
    train_y=train_y,
    valid_pose_x=valid_pose_x,
    valid_biomech_x=valid_biomech_x,
    valid_y=valid_y,
    adj=graph.AD,
    adj2=graph.AD2,
    adj3=graph.AD3,
...
# Make predictions
y_pred_prob = trainer.predict(valid_pose_x, valid_biomech_x)
...
# Additional visualization for feature importance (optional)
plt.figure(figsize=(10, 5))
plt.title('Feature Contributions')
plt.xlabel('Epoch')
plt.ylabel('Feature Importance')
save_and_show(fig_dir, 'feature_importance.png')